...
describe('Adapter#updateAll', () => {
  ...
  it('should update multiple users', async function () {
    ...
    assert.equal(users[0].name, 'John')
    assert.equal(users.filter(x => x[User.idAttribute] === userId1).length, 1)
    assert.equal(users.filter(x => x[User.idAttribute] === userId2).length, 1)
    ...
    assert.equal(users2[0].name, 'Johnny')
    assert.equal(users2.filter(x => x[User.idAttribute] === userId1).length, 1)
    assert.equal(users2.filter(x => x[User.idAttribute] === userId2).length, 1)
    assert.equal(users2.filter(x => x.age === 20).length, 1)
    assert.equal(users2.filter(x => x.age === 30).length, 1)
    ...
    assert.equal(users4[0].name, 'Johnny')
    assert.equal(users4.filter(x => x[User.idAttribute] === userId1).length, 1)
    assert.equal(users4.filter(x => x[User.idAttribute] === userId2).length, 1)
    assert.equal(users4.filter(x => x.age === 20).length, 1)
    assert.equal(users4.filter(x => x.age === 30).length, 1)
  })
})
