import { assert, options } from './index'
...
describe('Adapter#findAll', () => {
  ...
  if (options.hasFeature('findAllBelongsTo')) {
    ...
    it('should load belongsTo relations and filter sub queries', async function () {
      ...
      assert.debug('findAll', User.name, { [User.idAttribute]: user[User.idAttribute] })
      users = await adapter.findAll(User, { [User.idAttribute]: user[User.idAttribute] }, {
        with: [{
          relation: 'post',
          query: {
            status: 'published'
  ...
  if (options.hasFeature('filterOnRelations')) {
    it('should filter using belongsTo relation', async function () {
      ...
      const user1 = await adapter.create(User, { name: 'John', profileId: profile1.id })

      const post1 = await adapter.create(Post, { content: 'foo', userId: user1.id })
      ...
      const post2 = await adapter.create(Post, { content: 'bar', userId: user2.id })
      await adapter.create(Comment, { content: 'test2', postId: post2.id, userId: post2.userId })

      const users = await adapter.findAll(User, { 'profile.email': 'foo@test.com' })
    ...
    it('should filter through multiple hasOne/belongsTo relations', async function () {
      ...
      const user2 = await adapter.create(User, { name: 'Sally', profileId: profile2.id })
      const post2 = await adapter.create(Post, { content: 'bar', userId: user2.id })
      await adapter.create(Comment, { content: 'test2', postId: post2.id, userId: post2.userId })

      const comments = await adapter.findAll(Comment, { 'user.profile.email': 'foo@test.com' })
    ...
    it('should filter using multiple hasOne/belongsTo relations', async function () {
      ...
      const comments = await adapter.findAll(Comment, { 'user.name': 'John', 'user.profile.email': 'foo@test.com' })
      assert.equal(comments.length, 1)
      assert.equal(comments[0].userId, user1.id)
      assert.equal(comments[0].content, 'test1')
    })
