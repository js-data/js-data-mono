...
import { belongsToType, hasManyType, hasOneType } from './Relation';
...
export default class SimpleStore extends Container {
  ...
  destroyAll(name, query?, opts: any = {}) {
    return super.destroyAll(name, query, opts).then(result => {
      ...
      const hash = this.hashQuery(name, query);
  ...
  removeAll(name, query?, opts?: any) {
    if (!query || !Object.keys(query).length) {
      ...
      this._completedQueries[name][this.hashQuery(name, query)] = undefined;
    }
    const records = this.getCollection(name).removeAll(query, opts);
    if (records.length) {
