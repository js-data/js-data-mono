import { Component, Mapper, utils } from '@js-data/js-data';
...
export const withoutRelations = (mapper, props, opts: any = {}) => {
  opts.with = opts.with || [];
  const relationFields = mapper.relationFields || [];
  const toStrip = relationFields.filter(
    value => opts.with.indexOf(value) === -1
  );
  return utils.omit(props, toStrip);
};

export const reserved = ['orderBy', 'sort', 'limit', 'offset', 'skip', 'where'];
...
export class Response {
  ...
  constructor(
    ...
    meta = {},
    /**
     * op: operation for which the response was created.
     */
    public op: string
  ) {
...
export abstract class Adapter extends Component {
  ...
  count(mapper, query: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeCount(mapper, query, opts))
      .then(() => {
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterCount(mapper, query, opts, response))
  ...
  create(mapper, props = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeCreate(mapper, props, opts))
      .then(_props => {
        // Allow for re-assignment from lifecycle hook
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterCreate(mapper, props, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  createMany(mapper, props: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeCreateMany(mapper, props, opts))
      .then(_props => {
        // Allow for re-assignment from lifecycle hook
        props = _props === undefined ? props : _props;
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterCreateMany(mapper, props, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  destroy(mapper, id, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeDestroy(mapper, id, opts))
      .then(() => {
        op = opts.op = 'destroy';
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterDestroy(mapper, id, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  destroyAll(mapper, query: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeDestroyAll(mapper, query, opts))
      .then(() => {
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterDestroyAll(mapper, query, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  loadBelongsTo(mapper, def, records, __opts) {
    ...
    if (utils.isObject(records) && !utils.isArray(records)) {
      ...
      return this.find(
        relationDef,
        this.makeBelongsToForeignKey(mapper, def, record),
        __opts
      ).then(relatedItem => {
      ...
      return this.findAll(
        ...
        records.forEach(record => {
          relatedItems.forEach(relatedItem => {
            if (
              relatedItem[relationDef.idAttribute] === record[def.foreignKey]
            ) {
              def.setLocalField(record, relatedItem);
            }
          });
  ...
  find(mapper, id, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeFind(mapper, id, opts))
      .then(() => {
        op = opts.op = 'find';
        this.dbg(op, mapper, id, opts);
      ...
      .then(([record, meta]) => {
        ...
        return utils
          .resolve(this.afterFind(mapper, id, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  findAll(mapper, query, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeFindAll(mapper, query, opts))
      .then(() => {
        op = opts.op = 'findAll';
        this.dbg(op, mapper, query, opts);
      ...
      .then(([records, meta]) => {
        ...
        return utils
          .resolve(this.afterFindAll(mapper, query, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  loadRelationsFor(mapper, results, opts) {
    ...
    if (records) {
      utils.forEachRelation(mapper, opts, (def, __opts) => {
        ...
        if (
          def.foreignKey &&
          (def.type === 'hasOne' || def.type === 'hasMany')
        ) {
          if (def.type === 'hasOne') {
  ...
  getOpt(opt, opts: any = {}) {
    return opts[opt] === undefined
      ? utils.plainCopy(this[opt])
      : utils.plainCopy(opts[opt]);
  }
  ...
  loadHasMany(mapper, def, records, __opts) {
    ...
    const IDs = records.map(record =>
      this.makeHasManyForeignKey(mapper, def, record)
    );
    const query = {
      where: {}
    };
    ...
    return this.findAll(def.getRelation(), query, __opts).then(relatedItems => {
      records.forEach(record => {
        ...
        if (singular) {
          ...
          relatedItems.forEach(relatedItem => {
            if (
              utils.get(relatedItem, def.foreignKey) ===
              record[mapper.idAttribute]
            ) {
  ...
  loadHasManyLocalKeys(mapper, def, records, __opts) {
    ...
    if (record) {
      ...
      let localKeys = [];
      records.forEach(record => {
        localKeys = localKeys.concat(
          this.makeHasManyLocalKeys(mapper, def, record)
        );
      });
      return this.findAll(
        relatedMapper,
        ...
        records.forEach(item => {
          ...
          relatedItems.forEach(relatedItem => {
            if (
              itemKeys &&
              itemKeys.indexOf(relatedItem[relatedMapper.idAttribute]) !== -1
            ) {
              attached.push(relatedItem);
            }
          });
    ...
    if (record) {
      ...
      return this.findAll(
        relatedMapper,
        {
          where: {
            [def.foreignKeys]: {
              isectNotEmpty: records.map(record =>
                this.makeHasManyForeignKeys(mapper, def, record)
              )
            }
          }
        },
  ...
  sum(mapper, field, query: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeSum(mapper, field, query, opts))
      .then(() => {
        // Allow for re-assignment from lifecycle hook
        op = opts.op = 'sum';
      ...
      .then(([data, result]) => {
        ...
        op = opts.op = 'afterSum';
        return utils
          .resolve(this.afterSum(mapper, field, query, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  update(mapper, id, props: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeUpdate(mapper, id, props, opts))
      .then(_props => {
        // Allow for re-assignment from lifecycle hook
        props = _props === undefined ? props : _props;
        props = withoutRelations(mapper, props, opts);
      ...
      .then(([data, result = {}]) => {
        ...
        return utils
          .resolve(this.afterUpdate(mapper, id, props, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  updateAll(mapper, props: any = {}, query: any = {}, opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeUpdateAll(mapper, props, query, opts))
      .then(_props => {
        // Allow for re-assignment from lifecycle hook
        props = _props === undefined ? props : _props;
        props = withoutRelations(mapper, props, opts);
      ...
      .then(([data, result]) => {
        ...
        return utils
          .resolve(this.afterUpdateAll(mapper, props, query, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
  ...
  updateMany(mapper, records = [], opts: any = {}) {
    ...
    return utils
      .resolve(this.beforeUpdateMany(mapper, records, opts))
      .then(_records => {
        // Allow for re-assignment from lifecycle hook
      ...
      .then(results => {
        ...
        return utils
          .resolve(this.afterUpdateMany(mapper, records, opts, response))
          .then(_response => (_response === undefined ? response : _response));
      });
  }
}
